...
// vi g[N];
//=======================
//Books exchange (HARDER)
std::vector<bool> visited(100000 + 1, false);
std::vector<int> colour(100000 + 1);
std::vector<int> answer(100000 + 1, 0);
std::vector<int > adj(100000 + 1);
int tim = 0;
int dfs(int node, int parent)
{
	if (node == parent)
	{
		return answer[node] = tim;
	}
	tim++;

	return answer[node] = dfs(adj[node], parent);
...
void sol()
{
	int n;
	cin >> n;
	adj.resize(n + 1), answer.resize(n + 1);
