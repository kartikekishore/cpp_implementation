...
// vi g[N];
//=======================
//Dp Armchair codeforces
int n;
int dp[5000 + 5][5000 + 5];
std::vector<int> occupied, unoccupied;
int fun(int ui, int oi)
{
    if (ui == unoccupied.size() && oi < occupied.size() )return inf;
    if ( oi == occupied.size() )return 0;
    if (dp[ui][oi] != -1)return dp[ui][oi];
    return dp[ui][oi] = min(fun(ui + 1, oi + 1) + abs(occupied[oi] - unoccupied[ui]), fun(ui + 1, oi));
}
void sol()
{
