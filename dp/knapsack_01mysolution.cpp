...
// vi g[N];
//=======================
// my solved knapsack problem;
// i learned to always add +1 to the size of the matrix
int n, weight;
int dp[100000 + 1][100 + 1];
int fun(int i, int curw, int w[], int v[])
{
    if (dp[curw][i] != -1)
        return dp[curw][i];
    if (i >= n || curw > weight)
        return 0;
    if (curw + w[i] <= weight)
        return dp[curw][i] = max(fun(i + 1, curw + w[i], w, v) + v[i], fun(i + 1, curw, w, v));
    else
        return dp[curw][i] = fun(i + 1, curw, w, v);
