...
int knapsack(int Weight, int n)
{
	...
	if (Weight - w[n] < 0)
		return dp[n][Weight] = knapsack(Weight,  n - 1);
	else
		return dp[n][Weight] = max(knapsack(Weight - w[n],  n - 1) + v[n], knapsack(Weight,  n - 1));
}
void sol()
{
	int i, j, n;
