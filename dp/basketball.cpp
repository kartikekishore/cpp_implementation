...
int n;
int dp[100000 + 1][3];
int fun(int a[], int b[], int prev, int cur)
{
	if (cur >= n)
		return 0;
	if (dp[cur][prev] != -1)
	...
	int ans1 = max(fun(a, b, t, cur + 1) + cost, fun(a, b, prev, cur + 1));
	int ans2 = 0;
	if (prev == 0)
	{
		ans2 = max({fun(a, b, 1, cur + 1) + a[cur], fun(a, b, 2, cur + 1) + b[cur], fun(a, b, prev, cur + 1)});
	}
	return dp[cur][prev] = max(ans1, ans2);
}
void sol()
{
