...
//int n k direction
// 0 left   1 - right
int  dp[1000][1000 + 1][2];
int n, k;
int fun(int i, int power, int direction)
{
    if (i >= n || i < 0)
    {
        return 0;
    }
    else if (dp[i][power][direction] != -1)
    {
        return dp[i][power][direction];
    }
    int ans1 = 0, ans2 = 0;
    if (power > 1)
    {
        if (direction == 1)
        {
            ans2 = (fun(i - 1, power - 1, 0) + 1) % mod;
        }
        else
        {
            ans2 = (fun(i + 1, power - 1, 1) + 1) % mod;
        }
    }
    if (direction == 1)
    {
        ans1 = fun(i + 1, power, 1);
    }
    else
    {
        ans1 = fun(i - 1, power, 0);
    }
    return dp[i][power][direction] = (ans1 + ans2) % mod;
}
void sol()
{
    cin >> n >> k;
