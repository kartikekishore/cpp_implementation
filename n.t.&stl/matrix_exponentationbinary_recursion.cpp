...
//=======================
#define N 101
// matrix exponentaion store it in ar
int ar[N][N], I[N][N];

void mmul(int a[][N], int b[][N], int dim)
{
	int i, j;
	int res[dim + 1][dim + 1];
	fo(i, dim)
	fo(j, dim)
	{
		res[i][j] = 0;
		int k;
		fo(k, dim)res[i][j] = (res[i][j] + (a[i][k] * b[k][j])) % mod ;
	}

	fo(i, dim)fo(j, dim)a[i][j] = res[i][j];

}
void mtpow(int A[][N], int dim, int n)
{
	int i, j;
	fo(i, dim)fo(j, dim) {if (i == j)I[i][j] = 1; else I[i][j] = 0;}
