...
void sol()
{
	...
	cin >> n >> q;
	adj.resize(n), cnt.assign(n, 0), depth.assign(n, 0);
	clr(bit);
	for (int i = 0; i < n; ++i)
	{
		cin >> a[i];
	...
	for (int i = 0; i < n; i++)
	{
		pos[vec[i]] = i;
		update(i + 1, a[vec[i ]]);
		if (i + 1 + cnt[vec[i]] <= n)update(i + 1 + cnt[vec[i]], -a[vec[i]]);
	}
	...
	while (q--)
	{
		...
		if (choice == 1)
		{
			...
			update(pos[node] + 1, val - a[node]);
			if (pos[node] + cnt[node] + 1 <= n)update(pos[node] + cnt[node] + 1, -(val - a[node]));
			a[node] = val;
		}
		else
		{
