...
//=======================
//segment tree
//minimum in the range and its frequency and modify a[pos] to val
std::vector<int> a;
...
void build(int id = 1, int l = 0, int r = n)
{
	...
	if (st[id * 2].F < st[id * 2 + 1].F)
	{
		st[id].F = st[id * 2].F;
		st[id].S = st[id * 2].S;
	}
	else if (st[id * 2].F > st[id * 2 + 1].F)
	{
		st[id].F = st[id * 2 + 1].F;
		st[id].S = st[id * 2 + 1].S;
	}
...
pair<int , int> query(int x, int y, int id = 1, int l = 0, int r = n)
{
	...
	int mid = (l + r) / 2;
	pair<int , int> l_ans = query(x, y, 2 * id, l, mid);
	pair<int , int> r_ans = query(x, y, 2 * id + 1, mid, r);
	if (l_ans.F < r_ans.F)
		return l_ans ;
	else if (l_ans.F > r_ans.F)
		return r_ans ;
	else
